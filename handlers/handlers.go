...
func (h Handler) CreateEntries(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := tx.Prepare("db_entries(id,entry_val) VALUES (?, ?)")
	if err != nil {
		tx.Rollback()
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
...
func (h Handler) GetEntries(w http.ResponseWriter, r *http.Request) {
	...
	defer rows.Close()
	//utils.Panic(err, "Failed to query data!")
	//TODO rewrite
	for rows.Next() {
		var entry models.Entry
...
func (h Handler) DeleteOne(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "Entry %s deleted!", ID)
}
...
func (h Handler) Update(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "Entry %s updated!", ID)
}
